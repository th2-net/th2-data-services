...
from typing import Callable, Dict, Generator, Iterable, List, Optional, Tuple, Union, Generic, TypeVar, Type

from treelib.exceptions import NodeIDAbsentError
from th2.data_services.event_tree.common_event_tree import CommonEventTree
from th2.data_services.event_tree.common_event_tree import Event
...
EventTreeType = TypeVar("EventTreeType", bound=CommonEventTree)  # Can be any subtype of EventTree.


class CommonEventTreeCollection(Generic[EventTreeType]):
    ...
    def __init__(self, driver: IETCDriver):
        ...
        """
        self._driver = driver
        self._roots: List[EventTreeType] = []
        self._parentless: Optional[List[EventTreeType]] = None
        # {parent_event_id: [event1, ..]}
        self._detached_nodes: Dict[Optional[str], List[dict]] = defaultdict(list)
        self._et_class = self.setup_et_class()

    def setup_et_class(self) -> Type[CommonEventTree]:
        return CommonEventTree

    def _detached_parent_ids(self):
        return self._detached_nodes.keys()

    def _build_parentless_trees(self) -> List[EventTreeType]:
        ...
        for id_ in stub_roots:
            ...
            tree = self._et_class(event_id=event_id, event_name=event_name, data=stub_event)
    ...
    def _build_trees(self, events_nodes: Dict[Optional[str], List[dict]]) -> None:
        ...
        for root_event in events_nodes[None]:  # None - is parent_event_id for root events.
            ...
            tree = self._et_class(event_name=event_name, event_id=event_id, data=root_event)
            roots.append(tree)
    ...
    def _fill_tree(
        self, events_store: Dict[Optional[str], List[dict]], current_tree: EventTreeType, parent_id: str
    ) -> None:
    ...
    def get_parentless_trees(self) -> List[EventTreeType]:
    ...
    def append_event(self, event: dict) -> None:
        ...
        if parent_event_id is not None:  # Is root?
            ...
            event_id, event_name = self._driver.get_event_id(event), self._driver.get_event_name(event)
            tree = self._et_class(event_id=event_id, event_name=event_name, data=event)
    ...
    def get_detached_events_iter(self) -> Generator[Event, None, None]:
        """Yields detached events."""
        for events in self._detached_nodes.values():
            for event in events:
                yield event

    def get_detached_events(self) -> List[Event]:
    ...
    def get_trees(self) -> List[EventTreeType]:
    ...
    def get_root_by_id(self, id) -> Event:
    ...
    def get_tree_by_id(self, id) -> EventTreeType:
    ...
    def get_all_events_iter(self) -> Generator[Event, None, None]:
    ...
    def get_children(self, id: str) -> Tuple[Event]:
    ...
    def get_children_iter(self, id: str) -> Generator[Event, None, None]:
    ...
    def get_parent(self, id: str) -> Event:
    ...
    def get_full_path(self, id: str, field: str = None) -> List[Union[str, Event]]:  # noqa: D412
    ...
    def get_ancestors(self, id: str) -> List[Event]:
    ...
    def find_ancestor(self, id: str, filter: Callable) -> Optional[Event]:
    ...
    def find(self, filter: Callable, stop: Callable = None) -> Optional[Event]:
    ...
    def get_subtree(self, id: str) -> EventTreeType:
    ...
    def get_parentless_tree_collection(self) -> "CommonEventTreeCollection":
