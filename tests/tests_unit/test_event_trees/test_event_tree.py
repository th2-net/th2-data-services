...
def test_setitem(events_tree_for_test: EventTree):
    tree = events_tree_for_test
    tree["A_id"] = {"new data": 123}
    assert tree["A_id"] == {"new data": 123}


def test_setitem_id_error(events_tree_for_test: EventTree):
    with pytest.raises(EventIdNotInTree) as exc:
        events_tree_for_test["test"] = "test"
    assert exc


def test_update_parent_link(events_tree_for_test: EventTree):
    tree = events_tree_for_test
    tree.update_parent_link("D1_id", "root_id")
    assert tree.get_parent("D1_id") == {"data": [1, 2, 3, 4, 5]}


def test_update_parent_link_id_error(events_tree_for_test: EventTree):
    with pytest.raises(EventIdNotInTree) as exc:
        events_tree_for_test.update_parent_link("test", "to_test")
    assert exc


def test_update_parent_link_loop_error(events_tree_for_test: EventTree):
...
def test_update_event_name(events_tree_for_test: EventTree):
    tree = events_tree_for_test
    tree.update_event_name("C_id", "NewEvent")
    assert tree._tree["C_id"].tag == "NewEvent"


def test_update_event_name_id_error(events_tree_for_test: EventTree):
    with pytest.raises(EventIdNotInTree) as exc:
