...
def test_preserve_body_is_notset(demo_events_from_data_source: Data):
    events_tree = ParentEventsTree(demo_events_from_data_source)
    with_body = set()
...
def test_preserve_body_is_false(demo_events_from_data_source: Data):
    events_tree = ParentEventsTree(demo_events_from_data_source, preserve_body=False)
    with_body = set()
    for v in events_tree.events.values():
        if v.get("body") is not None:
            with_body.add(True)
...
def test_preserve_body_is_true(demo_events_from_data_source: Data):
    ...
    events_tree = ParentEventsTree(demo_events_from_data_source, preserve_body=True)
    with_body = set()
    for v in events_tree.events.values():
        if v.get("body") is not None:
            with_body.add(True)
